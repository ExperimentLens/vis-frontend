...
export const buildTraceHourBuckets = (
  ...
  details.forEach(trace => {
    ...
    const existing = tracesByHour.get(hourKey) ?? [];

    tracesByHour.set(hourKey, [...existing, trace]);
  });
        ...
        .map(trace => {
          ...
          return `
            <div style="padding:8px 0;border-bottom:1px solid ${palette.border};">
              <div style="display:flex;gap:8px;align-items:flex-start;justify-content:space-between;">
                <div style="min-width:0;">
                  ...
                  <div style="font-size:0.66rem;color:${palette.secondaryText};margin-top:2px;">
                    ${sanitize(time)} · Workflow: ${buildWorkflowLink({ experimentId, workflowId, palette })}
                  </div>
                </div>
              ...
              <div style="display:grid;grid-template-columns:repeat(4, max-content);gap:4px 12px;margin-top:8px;font-size:0.66rem;">
